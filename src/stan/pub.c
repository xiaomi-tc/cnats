...
stanProcessPubAck(natsConnection *nc, natsSubscription *sub, natsMsg *msg, void *closure)
{
    ...
    if (dataLen > 0)
    {
        Pb__PubAck *pubAck = pb__pub_ack__unpack(NULL, dataLen, data);
        ...
        if (pubAck != NULL)
        {
            ...
            pb__pub_ack__free_unpacked(pubAck, NULL);
        }
    }
...
_stanPublish(stanConnection *sc, const char *channel, const void *data, int dataLen,
        char *guidBuf, int guidBufLen, bool isSync, int64_t *deadline, _pubAck *pa)
{
    ...
    if (guidBufLen < STAN_GUID_BUF_LEN)
        return nats_setError(NATS_INSUFFICIENT_BUFFER, "The guid buffer size nees to be at least %d bytes, it is only %d bytes", STAN_GUID_BUF_LEN, guidBufLen);
    ...
    s = natsNUID_Next(guidBuf, guidBufLen);
    if (s == NATS_OK)
    {
        Pb__PubMsg  pubReq;
        ...
        else
        {
            ...
            if (s == NATS_OK)
            {
                ...
                if (s == NATS_OK)
                {
                    ...
                    // For Publish() calls, store in map, make a copy of the guid for the key
                    if (isSync)
                    {
                        s = natsStrHash_Set(sc->pubAckMap, guidBuf, true, (void*) pa, NULL);
                    }
                    else
                    {
                        pa->deadline = ackTimeout;
                        // For PublishAsync() calls, we need to keep a copy in `pa`, so make
                        // the copy and use it as key without having to copy the key.
                        DUP_STRING(s, pa->guid, guidBuf);
                        if (s == NATS_OK)
            ...
            if (s == NATS_OK)
            {
                ...
                if ((s != NATS_OK) && (pa != NULL))
                {
                    ...
                    natsMutex_Lock(sc->pubAckMu);
                    natsStrHash_Remove(sc->pubAckMap, guidBuf);
...
stanConnection_Publish(stanConnection *sc, const char *channel, const void *data, int dataLen)
{
    ...
    s = _stanPublish(sc, channel, data, dataLen, guid, (int) sizeof(guid), true, &deadline, pa);
    if (s == NATS_OK)
    {
        bool received = false;
        bool closed   = false;
        ...
        if (s != NATS_OK)
        {
            ...
            if (natsStrHash_Remove(sc->pubAckMap, guid) != NULL)
            {
...
natsStatus
stanConnection_PublishAsync(stanConnection *sc, char *guidBuf, int guidBufLen,
        const char *channel, const void *data, int dataLen,
        stanPubAckHandler ah, void *ahClosure)
{
    ...
    s = _stanPublish(sc, channel, data, dataLen, guidBuf, guidBufLen, false, NULL, pa);
