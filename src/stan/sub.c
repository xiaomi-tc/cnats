...
_stanProcessMsg(natsConnection *nc, natsSubscription *ignored, natsMsg *msg, void *closure)
{
    ...
    pbMsg = pb__msg_proto__unpack(NULL, (size_t) msg->dataLen, (const uint8_t*) msg->data);
    if (pbMsg == NULL)
    {
        natsMsg_Destroy(msg);
        return;
    ...
    pb__msg_proto__free_unpacked(pbMsg, NULL);
}
        ...
        stanSubOptions *opts)
{
    ...
    if (s == NATS_OK)
    {
        ...
        if (s == NATS_OK)
        {
            ...
            {
                subReq.starttimedelta = nats_NowInNanoSeconds() - sub->opts->startTime;
            }
