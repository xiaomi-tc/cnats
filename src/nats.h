...
/** \brief The minimum size of a GUID buffer
 *
 * When calling #stanConnection_PublishAsync(), the user needs to provide
 * a buffer which the function copies a generated GUID into. This is the
 * minimum size the buffer needs to be.
 */
#define STAN_GUID_BUF_LEN   (23)
#endif
...
typedef void (*stanConnectionLostHandler)(
        stanConnection *sc, natsStatus s, const char* errorTxt, void *closure);
#endif
 ...
 * is expressed in number of nanoseconds since the EPOCH, as given by
 * nats_NowInNanoseconds() for instance.
 *
 ...
 *
 * @param opts the pointer to the #stanSubOptions object.
 * @param time the start time, expressed in nanoseconds since the EPOCH.
 */
NATS_EXTERN natsStatus
 ...
 * this means start receiving messages that were sent n seconds ago.
 *
 * The delta is expressed in nanoseconds.
 *
 * @param opts the pointer to the #stanSubOptions object.
 ...
 * comes. The user can correlate the acknowledgment with the GUID returned by
 * this function.
 *
 * @param guidBuf the buffer the GUID is copied to. The GUID will be `NULL` terminated.
 * @param guidBufLen the size of the provided buffer. This buffer needs to be
 * at least STAN_GUID_BUF_LEN long.
